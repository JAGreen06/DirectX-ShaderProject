...
ID3D11DepthStencilState* gLessEqualDepthState = nullptr;
...
bool CreateStates()
{
    ...
    // Create a DirectX object for the description above that can be used by a shader
    if (FAILED(gD3DDevice->CreateDepthStencilState(&depthStencilDesc, &gLessEqualDepthState)))
    {
        gLastError = "Error creating depth-read-only state";
        return false;
    }
...
void ReleaseStates()
{
    ...
    if (gLessEqualDepthState)    gLessEqualDepthState->Release();
