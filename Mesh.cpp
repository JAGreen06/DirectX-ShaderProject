...
void Mesh::RenderRecursive(std::vector<CMatrix4x4>& modelMatrices, unsigned int nodeIndex /*= 0*/, CMatrix4x4 parentWorldMatrix /*= MatrixIdentity()*/)
{
    ...
    for (auto childIndex : mNodes[nodeIndex].childNodes)
    {
        RenderRecursive(modelMatrices, childIndex, nodeWorldMatrix);
    }
}
...
void Mesh::Render(std::vector<CMatrix4x4>& modelMatrices)
{
    ...
    for (unsigned int nodeIndex = 0; nodeIndex < mNodes.size(); ++nodeIndex)
    {
        ...
        if(mNodes[nodeIndex].parentIndex != nodeIndex)
        {
            thisNode.absoluteMatrix *= mNodes[thisNode.parentIndex].absoluteMatrix;
		}
        else
        {
			thisNode.absoluteMatrix = modelMatrices[nodeIndex];
